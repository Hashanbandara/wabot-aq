...
let handler  = async (m, { conn, args, usedPrefix, command }) => {
  ...
  if ((args[0] && args[0] == 'plz') || global.conn.user.jid == conn.user.jid) {
    ...
    conn.on('qr', async qr => {
      let scan = await parent.sendFile(m.chat, await qrcode.toDataURL(qr, { scale: 8 }), 'qrcode.png', '*👇ඔබටත් බොට් කෙනෙක් වීමට අවශ්‍යනම් පහත සදහන් විදිහට සිදු කරන්න👇* 1. වට්ස්සැප් එක ඔපන් කර දකුනු පැත්තෙ කෙලවරේ ඉහල ඇති තිත් තුන ක්ලික් කරන්න 2. ඉන්පසු WhatsApp Web ක්ලික් කරන්න\n3. දැන් qr coad එක ස්කෑන් කරන්න \nQR *තප්පර 20 qr coad එක e.x.p වෙනවා ඊට කලින් WHATSAPP WEB එකේ SCAN කරන්න🇱🇰*', m)
      setTimeout(() => {
